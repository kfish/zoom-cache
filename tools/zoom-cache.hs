...
zoomInfoHandler :: App () ()
zoomInfoHandler = mapM_ (liftIO . zoomInfoFile defaultMappings) =<< appArgs
...
zoomDumpHandler = do
    (config, filenames) <- liftIO . processArgs =<< appArgs
    mapM_ (liftIO . zoomDumpFile defaultMappings (track config)) filenames
...
zoomSummaryHandler = do
    ...
    where
        f trackNo (lvl:paths) = mapM_ (zoomDumpSummaryLevel defaultMappings trackNo (read lvl)) paths
